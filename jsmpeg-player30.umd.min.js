    ...
    var k = function() {
        ...
        G = function() {
            ...
            return t.startLoading = function() {
                ...
                // İlk çalıştırma - başlangıç zamanını kaydet
                ...
                // Başlangıçtan bu yana geçen gerçek süre (saniye)
                ...
                if (this.audio && this.audio.canPlay && this.video) {
                    ...
                    // === VIDEO ===
                    // Video'yu gerçek zamana göre decode et
                    // Hedef: totalVideoDecoded ≈ realElapsed
                    var videoTargetTime = realElapsed;
                    var maxCatchUp = 5; // Bir seferde max 5 frame
                    var videoCatchUp = 0;
                    
                    while (this.totalVideoDecoded < videoTargetTime && videoCatchUp < maxCatchUp) {
                        var prevVidTime = this.video.currentTime;
                        if (!this.video.decode()) break;
                        var decodedVid = this.video.currentTime - prevVidTime;
                        if (decodedVid > 0) this.totalVideoDecoded += decodedVid;
                        else this.totalVideoDecoded += frameTime; // Fallback
                        videoCatchUp++;
                    }
                    
                    // === SYNC CHECK ===
                    // Audio oynatma zamanı (gerçek)
                    var audioPlayTime = this.totalAudioDecoded - this.audioOut.enqueuedTime;
                    var drift = audioPlayTime - this.totalVideoDecoded;
                    
                    // Debug - büyük drift varsa logla
                    if (Math.abs(drift) > 1.0 && this.options.onSyncReset) {
                        this.options.onSyncReset({reason: 'drift_warning', drift: drift, audio: audioPlayTime, video: this.totalVideoDecoded});
                    }
