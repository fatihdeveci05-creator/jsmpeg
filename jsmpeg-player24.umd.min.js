    ...
    var k = function() {
        ...
        G = function() {
            ...
            return t.startLoading = function() {
                ...
                var elapsedSinceStart = now - this.streamingStartTime;
                
                if (this.audio && this.audio.canPlay && this.video) {
                    var targetBuf = this.options.targetAudioBuffer || 0.5;
                    var maxDrift = this.options.maxSyncDrift || 0.3;
                    var hardLimit = this.options.hardSyncLimit || 1.0;
                    ...
                    // === VIDEO DECODE - Frame rate kontrolü ===
                    // Video'yu gerçek zamana göre decode et
                    var timeSinceLastVideo = now - this.lastVideoDecodeTime;
                    var shouldDecodeVideo = timeSinceLastVideo >= frameTime;
                    
                    // Drift hesapla
                    var audioPlayTime = this.audio.decodedTime - this.audioOut.enqueuedTime;
                    var videoTime = this.video.currentTime;
                    var drift = audioPlayTime - videoTime;
                    ...
                    // === SYNC MANTIĞI ===
                    if (drift > maxDrift && shouldDecodeVideo) {
                        // Audio önde - video'yu hızlandır (fazla frame decode et)
                        var catchUp = Math.min(Math.ceil(drift / frameTime), 3);
                        for (var j = 0; j < catchUp; j++) { 
                            this.video.decode();
                        }
                        this.lastVideoDecodeTime = now;
                    } else if (drift < -maxDrift) {
                        // Audio geride - video'yu DURDUR (decode etme)
                        // Video beklesin, audio yakalaşsın
                    } else if (shouldDecodeVideo) {
                        // Normal - video decode et (frame rate'e uygun)
                        this.video.decode();
                        this.lastVideoDecodeTime = now;
                    }
                    ...
                    if (!this.lastVideoDecodeTime) this.lastVideoDecodeTime = now;
                    var el = now - this.lastVideoDecodeTime;
                    if (el >= frameTime2) { this.video.decode(); this.lastVideoDecodeTime = now; }
                }
