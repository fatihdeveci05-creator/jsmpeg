        ...
        G = function() {
            ...
            return t.startLoading = function() {
                ...
                if (this.audio && this.audio.canPlay && this.video) {
                    ...
                    var frameRate = this.video.frameRate || 22;
                    var frameTime = 1 / frameRate;
                    var now = Q();
                    if (!this.lastUpdateTime) this.lastUpdateTime = now;
                    var elapsed = now - this.lastUpdateTime;
                    this.lastUpdateTime = now;
                    var wasStalled = elapsed > 0.5 || this.needsSyncReset;
                    this.needsSyncReset = false;
                    var decoded = 0;
                    while (this.audioOut.enqueuedTime < targetBuf && decoded < 10) {
                        if (!this.audio.decode()) break;
                        decoded++;
                    }
                    var audioTime = this.audio.decodedTime - this.audioOut.enqueuedTime;
                    var videoTime = this.video.currentTime;
                    var drift = audioTime - videoTime;
                    var absDrift = Math.abs(drift);
                    if (absDrift > hardLimit || wasStalled) {
                        this.audioOut.resetEnqueuedTime();
                        this.audio.decodedTime = this.video.currentTime;
                        for (var i = 0; i < 5; i++) { if (!this.audio.decode()) break; }
                        this.video.decode();
                        if (this.options.onSyncReset) this.options.onSyncReset({reason: wasStalled ? 'stall' : 'drift', drift: drift});
                        return;
                    }
                    if (drift > maxDrift) {
                        var catchUp = Math.min(Math.ceil(drift / frameTime), 5);
                        for (var j = 0; j < catchUp; j++) { if (!this.video.decode()) break; }
                    ...
                    var frameTime2 = 1 / (this.video.frameRate || 22);
