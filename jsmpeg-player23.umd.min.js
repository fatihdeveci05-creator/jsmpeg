        ...
        G = function() {
            ...
            return t.startLoading = function() {
                ...
                if (this.audio && this.audio.canPlay && this.video) {
                    ...
                    var audioSpeedMult = this.options.audioSpeedMultiplier || 1.0;
                    ...
                    // Audio buffer'ı doldur - audioSpeedMultiplier ile ayarla
                    var baseDecodeCount = 10;
                    var adjustedDecodeCount = Math.round(baseDecodeCount * audioSpeedMult);
                    var decoded = 0;
                    while (this.audioOut.enqueuedTime < targetBuf && decoded < adjustedDecodeCount) {
                        if (!this.audio.decode()) break;
                        decoded++;
                    }
                    
                    // Ekstra audio decode - hız çarpanına göre
                    if (audioSpeedMult > 1.0) {
                        var extraDecodes = Math.round((audioSpeedMult - 1.0) * 20);
                        for (var e = 0; e < extraDecodes; e++) {
                            if (!this.audio.decode()) break;
                        }
                    }
                    ...
                    // Hard reset - büyük kayma veya stall
                    ...
                    // Sync mantığı
                    if (drift > maxDrift) {
                        // Audio önde - video'yu hızlandır
                        var catchUp = Math.min(Math.ceil(drift / frameTime), 5);
                        for (var j = 0; j < catchUp; j++) { if (!this.video.decode()) break; }
                    } else if (drift < -maxDrift) {
                        // Audio geride - video'yu DURDUR, fazla audio decode et
                        var audioCatchUp = Math.min(Math.ceil(-drift * 40), 10);
                        for (var k = 0; k < audioCatchUp; k++) { 
                            if (!this.audio.decode()) break; 
                        }
                        // Video decode ETME - audio yakalaşsın
                    } else {
                        // Normal - video decode et
                        this.video.decode();
                    }
